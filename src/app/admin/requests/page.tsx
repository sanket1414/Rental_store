...
import { getRequests, updateRequest, getProductById, addInvoice, getCustomers } from '@/lib/data-store';
...
export default function AdminRequestsPage() {
    ...
    const handleStatusChange = async (status: RequestStatus) => {
        ...
        await updateRequest(selected.id, { status, quotedPrice: price, adminNotes: editNotes });
        await reload();
        setSelected(null);
    ...
    const handleSaveNotes = async () => {
        if (!selected) return;
        const price = editPrice === '' ? 0 : editPrice;
        await updateRequest(selected.id, { quotedPrice: price, adminNotes: editNotes });
        await reload();
        alert('Price and notes saved!');
    };
    ...
    const handleCreateInvoice = async () => {
        ...
        await addInvoice({
            ...
            items: [{
                ...
                pricePerDay: price / (selected.daysRequired || 1),
                total: price,
            }],
            subtotal: price,
            discount: 0,
        ...
        alert('Invoice created! Go to Invoices tab to view.');
    };
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} onClick={(e) => e.stopPropagation()} className="bg-[#121212] rounded-2xl border border-gray-800 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="p-6 border-t border-gray-800 flex flex-wrap gap-2">
                                ...
                                <button onClick={handleCreateInvoice} className="flex items-center gap-2 bg-purple-900/30 text-purple-400 px-4 py-2 rounded-xl text-sm font-medium hover:bg-purple-900/50 transition-all ml-auto"><FileText size={16} /> Create Invoice</button>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
